# ============================================================================
# 0001 - AFAR ULTIMATE v6.0 – Fully Bilingual (Dynamic Language Switching)
# ============================================================================
"""
Author: AI Senior Developer (INTJ/ENTJ, 15+ years experience)
License: MIT
Description: Complete dynamic bilingual support (EN/FA). Every text element
             ...
             right-click context menus, customizable colors, and full help.
"""
...
# ============================================================================
# 0003 - 50 REGEX PATTERNS (unchanged)
# ============================================================================
PATTERNS_50 = [
    r"^[├│└└──\s├──]+", r"^[└──\s│]+", r"^[─\s│├└]+", r"^[│\s]+",
...
# ============================================================================
# 0005 - BILINGUAL DICTIONARY (Complete – all strings translated)
# ============================================================================
T = {
    "en": {
        "title": "⚡ AFAR PRO v2.0 – Build & Reverse",
        "ready": "Ready",
        "tab_build": "📥 Build",
        "tab_reverse": "📤 Reverse",
        "tab_help": "📖 Guide",
...
• Check Live Log for errors"""
    ...
    "fa": {
        "title": "⚡ AFAR حرفه‌ای v2.0 – ساخت و معکوس",
        "ready": "آماده",
        "tab_build": "📥 ساخت",
        "tab_reverse": "📤 معکوس",
        "tab_help": "📖 راهنما",
...
# ============================================================================
# 0006 - CONTEXT MENU (Right-Click) – dynamically uses current language
# ============================================================================
def add_context_menu(widget, lang):
    """Adds right-click context menu with current language."""
...
class AFAR:
    def __init__(self, root):
        ...
        # Core variables (StringVars preserve state across UI rebuilds)
        ...
        self.build_ui()
        self.create_credit_footer()   # متن اعتباری در جای مشخص‌شده (زیر هدر)
    ...
    def save_config(self):
        ...
        with open(cfg, 'w') as f:
            json.dump({"lang": self.lang, "theme": self.theme}, f, indent=2)

    # ------------------------------------------------------------------------
    # 0009 - THEME APPLICATION
    # ------------------------------------------------------------------------
        ...
        def rec(w):
            try:
                if isinstance(w, (tk.Frame, tk.LabelFrame, tk.PanedWindow)):
                    ...
                    w.configure(bg=self.theme["panel"] if w.master != self.root else self.theme["bg"],
                                fg=self.theme["fg"])
    ...
    # ------------------------------------------------------------------------
    # 0010 - UI BUILD (creates all widgets from scratch)
    # ------------------------------------------------------------------------
    def build_ui(self):
        # Header
    ...
    # ------------------------------------------------------------------------
    # 0011 - REFRESH UI (rebuilds everything with new language)
    # ------------------------------------------------------------------------
    def refresh_ui(self):
        """Destroy all UI widgets and rebuild with current language."""
        # Save state of checkboxes before destroying
        readme_val = self.cb_readme.get()
        gitkeep_val = self.cb_gitkeep.get()
        meta_val = self.cb_meta.get()

        # Destroy everything except the root window and its children
        for child in self.root.winfo_children():
            child.destroy()

        # Rebuild UI
        self.build_ui()
        self.create_credit_footer()   # بازسازی متن با زبان جدید
        self.create_status_bar()
        self.create_log()
        self.apply_theme()
        self.update_status_lang()
        ...
        # Update the actual checkbuttons if they exist
        if hasattr(self, 'cb_readme_widget'):
            self.cb_readme_widget.select() if readme_val else self.cb_readme_widget.deselect()
        if hasattr(self, 'cb_gitkeep_widget'):
            self.cb_gitkeep_widget.select() if gitkeep_val else self.cb_gitkeep_widget.deselect()
        if hasattr(self, 'cb_meta_widget'):
            self.cb_meta_widget.select() if meta_val else self.cb_meta_widget.deselect()
    ...
    def init_reverse(self):
        ...
        ee = tk.Entry(cfg, textvariable=self.spec, bg=self.theme["input"], fg=self.theme["fg"], font=("Consolas", 10), bd=1, relief=tk.SOLID)
        ee.pack(fill=tk.X, padx=8, pady=4)
        add_context_menu(ee, self.lang)
...
"""
    ...
    # ========================================================================
    # CREDIT FOOTER – دقیقاً زیر نوار عنوان و وسط‌چین (همان‌طور که در تصویر مشخص شده)
    # ========================================================================
    def create_credit_footer(self):
        """نمایش متن اعتباری در بالای صفحه، زیر هدر و وسط‌چین با لینک کلیک‌پذیر"""
        # فریم اصلی که زیر هدر قرار می‌گیرد
        credit_frame = tk.Frame(self.root, bg=self.theme["panel"])
        credit_frame.pack(side=tk.TOP, fill=tk.X, pady=(2, 4))

        # زیر-فریم برای وسط‌چین کردن محتوا
        inner = tk.Frame(credit_frame, bg=self.theme["panel"])
        inner.pack(anchor="center")

        # بخش ثابت
        txt = tk.Label(inner, text="Developed by", bg=self.theme["panel"],
                       fg=self.theme["fg"], font=("Segoe UI", 9, "italic"))
        txt.pack(side=tk.LEFT, padx=(0, 5))
